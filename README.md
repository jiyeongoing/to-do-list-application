...
- Node 기본 테스트 러너로 주요 동작 검증
- GitHub Pages와 Supabase 무료 플랜을 활용한 계정·동기화 구조
...
- Constraint: Free hosting, local-first cloud sync
...
공개 데모: https://jiyeongoing.github.io/to-do-list-application/
...
- 기기 저장, 회원 계정 동기화, JSON 백업/복원

비로그인 상태에서는 브라우저 기기의 `localStorage`에 저장합니다. Supabase 연결 후
로그인하면 사용자별 클라우드 DB에 동기화됩니다.
...
| Storage | LocalStorage, Supabase Postgres, JSON Backup/Restore |
| Auth | Supabase Auth |
| Test | Node test runner |
| Optional Backend Prototype | Java 21, Spring Boot, Gradle, H2, JPA |
...
- [supabase/schema.sql](supabase/schema.sql): 사용자별 데이터 테이블 및 RLS 정책
...
Node 기본 테스트 러너로 주요 투두 흐름 22개를 검증합니다.
...
Supabase 무료 클라우드 연결 방법은 [docs/SUPABASE_SETUP.md](docs/SUPABASE_SETUP.md)를 참고합니다.

## Optional Backend Prototype

계정 동기화 확장을 검토한 Spring Boot 실험 코드는 [backend](backend) 폴더에 분리되어 있습니다.
무료 공개 버전에서는 연결하지 않습니다.
...
백엔드 실험에서는 비밀번호를 BCrypt 해시로 저장하고, 로그인 상태는 서버 세션으로 유지합니다.
로컬 실행에서는 H2 파일 DB(`backend/data`)를 사용합니다.
