...
- Demo: https://jiyeongoing.github.io/to-do-list-application/?v=19
...
- Node 기본 테스트 러너로 주요 동작 22개 케이스 검증
- Java/Spring Boot 로컬 계정 저장 API와 H2/JPA 저장 구조 추가
...
- Constraint: Guest-first, account sync prototype
...
공개 데모: https://jiyeongoing.github.io/to-do-list-application/?v=19
...
- 게스트 저장 유지, 이메일/비밀번호 계정 저장, Google 계정 연동, 이 기기 데이터 가져오기
...
| Local Backend | Java 21, Spring Boot, Gradle, H2, JPA |
...
로컬 회원가입/로그인/동기화 서버는 [backend](backend) 폴더에 있습니다.
...
이메일 중복확인, 이메일/비밀번호 회원가입, 별명 입력, 로그인을 지원합니다.
비밀번호는 원문이 아니라 BCrypt 해시로 저장하고, 로그인 상태는 서버 세션으로 유지합니다. 백엔드에 Google OAuth
client id/secret을 설정하면 같은 이메일 계정에 Google ID를 연결합니다. 서버가
꺼져 있어도 기존 로컬 저장 흐름은 유지됩니다.
