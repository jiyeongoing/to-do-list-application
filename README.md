...
공개 데모: https://jiyeongoing.github.io/to-do-list-application/?v=13
...
- 게스트 저장 유지, Google 로그인 프로토타입, 이 기기 데이터 가져오기
...
로컬 로그인/동기화 프로토타입 서버는 [backend](backend) 폴더에 있습니다.
...
계정 저장 데이터는 백엔드의 H2 파일 DB(`backend/data`)에 보관됩니다. `data/`는
개인 실행 데이터라 Git에는 올리지 않습니다.
